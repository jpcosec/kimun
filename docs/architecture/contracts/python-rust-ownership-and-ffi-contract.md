# Python/Rust Ownership and FFI Contract

## Purpose and governing sources

This contract defines the Python adapter boundary for deployments that keep a Python CLI shell around the Rust kernel. It does not make Python a required architectural center.
...
- **Workflow Composition**: Sequencing calls to the Rust kernel when a Python CLI is the chosen host.
...
### Permitted Data Flow
- **Python to Rust**: Command parameters, filesystem paths, raw document content for extraction.
- **Rust to Python**: Serialized AST segments, query result sets, structural error manifests, success/fail status.

### Prohibited Crossings
- **No Python Logic in Core**: Rust must not call back into Python for any canonical decision.
...
Other hosts may use Rust API, IPC, HTTP, or other adapter boundaries instead of PyO3.

## Non-Negotiable
If a task requires structural reasoning about a document, it **must** reside in Rust. If a task requires environmental side-effects (Git, subprocesses), it belongs in a host adapter outside canonical kernel authority; Python is one valid adapter, not the only one.
