...
def _diagnose_model(model_entry, root, pythonpath) -> tuple[ModelDiagnosis, ModelsIndex]:
...
def diagnose_store(store_path: Path, project_root: Optional[Path] = None, pythonpath: Optional[str] = None) -> StoreDiagnosis:
    ...
    diagnoses = [_diagnose_model(m, root, pythonpath) for m in store_index.models]
