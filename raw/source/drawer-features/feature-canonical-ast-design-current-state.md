...
## Extensibility principle for the AST and Clojure core
...
It is also the Clojure implementation.

The Clojure core should be designed so that new node types, edge kinds, semantic layers, anchoring modes, projection adapters, and indexing strategies can be added without collapsing the base model.

A useful implementation direction is to consider graph-oriented internal tooling such as `DataScript/Asami`.
...
This suggests a Clojure design where:
...
But we should avoid designing the Clojure core in a way that blocks that future.
