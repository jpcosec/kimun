---
id: task-specify-rust-canonical-core-contract
status: closed
summary: Re-anchored to high-fidelity contract in docs/architecture/contracts/rust-canonical-core-contract.md.
tags:
- workspace:desk
- artifact:task
- system:sldb
...
# Specify Rust Canonical Core Contract

## Rationale

Govened by `rust-core.md`. The Rust engine must serve as the "Canonical Truth," ensuring that every document segment has a deterministic identity and every relation is structurally validated. This task ensures we don't compromise on lossless parsing or identity hashing.
...
4. **Contract Materialization**: Write the final specification in `docs/architecture/contracts/rust-canonical-core-contract.md`.

## Validation

- Does the contract mandate the use of `Rowan` for lossless parsing?
- Does it require `Blake3` for content identity?
- Does it align with the `RustCore` block in `target-components.yml`?

## Done When

The high-fidelity `rust-canonical-core-contract.md` is committed and approved.
