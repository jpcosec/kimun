---
id: task-freeze-python-rust-ownership-and-ffi-contract
status: closed
summary: Re-anchored to high-fidelity contract in docs/architecture/contracts/python-rust-ownership-and-ffi-contract.md.
tags:
- workspace:desk
- artifact:task
- system:sldb
...
# Freeze Python/Rust Ownership and FFI Contract

## Rationale

Grounded in `decision-rust-core-with-minimal-python.md`. To achieve "Canonical Truth in Rust," we must strictly define where Python's orchestration ends and Rust's structural reasoning begins. This prevents the "leaky abstraction" of the previous architecture where Python managed AST state.
...
- **In-Scope**: 
  ...
  - **Rust Responsibility**: `CanonicalAST` management, `.sldb` persistence, `Blake3` hashing, and structural query execution.
...
4. **Contract Materialization**: Write the final specification in `docs/architecture/contracts/python-rust-ownership-and-ffi-contract.md`.
...
The `python-rust-ownership-and-ffi-contract.md` is frozen and bound to all implementation tasks.
