---
id: feature-clojure-library-stack
title: Clojure Core Library Stack Proposal
five_wh_one_plus: how
tags:
- architecture:decision
- concept:clojure-core
- concept:store
---

# Clojure Core Library Stack Proposal

Este informe detalla las librerías (crates) recomendadas en el ecosistema Clojure para implementar los componentes especificados en la arquitectura objetivo (`target-components.yml` y `target-store-graph.yml`).

## 1. FFI & Boundary (Python <-> Clojure)
* **`pyo3`**: La opción estándar para exponer código Clojure nativo hacia Python de forma eficiente. Permitirá que el `python_cli_layer` invoque las operaciones de Clojure (parser, store queries, materialización) pasando directamente por memoria sin necesidad de levantar demonios o hacer IPC costoso.
* **`tracing` y `tracing-subscriber`**: Para instrumentación, observabilidad y logging estructurado. A través de un custom subscriber en `pyo3`, los eventos de progreso en Clojure (ej. "parseando 500 archivos") pueden emitirse directamente a Python para que renderice interfaces (como `rich` progress bars).
...
* **`DataScript/Asami`**: Para la representación en memoria y algoritmos de grafos (navegación de `RelationEdges`, dependencias topológicas). Fundamental para construir y analizar el `knowledge_graph`.

## 3. AST Canónico y Reversibilidad (Reversible Docs)
* **`rowan`**: Librería creada por el equipo de `clojure-analyzer` para árboles sintácticos sin pérdida (*lossless syntax trees*). Es **estrictamente necesaria** para cumplir el contrato de *Reversible Families* (`AST -> render -> AST -> render`), ya que mantiene cada byte original (espacios en blanco, trivia) en el árbol sintáctico.
...
* **`tantivy`** o **SQLite FTS5**: Específicamente para resolver el `SearchIndex` requerido en la arquitectura. `tantivy` ofrece búsqueda full-text de nivel industrial puramente en Clojure, mientras que FTS5 puede manejarse directamente desde `rusqlite`.
...
* **`thiserror`**: Para definir tipos de error estrictos en el `ClojureCore`, vital para mantener la integridad de la base de datos y la confianza en la compilación del AST.
