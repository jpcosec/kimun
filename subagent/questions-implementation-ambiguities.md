...
1. **Rust/Python ownership map:** Which first-slice components are implemented in Rust vs Python orchestration? The atoms assign the canonical core to Rust and the user shell to Python, but do not explicitly place `markdown-importer`, `markdown-emitter`, `document-tracker`, `document-materializer`, `query-engine`, `store-infrastructure`, `store-integrity-checks`, or `semantic-indexing`. (`rust-core.md`, `python-cli-orchestration-layer.md`, related component atoms)
2. **Python↔Rust boundary contract:** What is the concrete integration contract between the Python CLI layer and the Rust core—API shape, serialization format, error taxonomy, and versioning of AST/projection payloads? Without this, orchestration can easily re-own core logic or tests can lock onto the wrong boundary. (`python-patterns.md`, `rust-patterns.md`, `ast-command-group.md`)
...
12. **Test ownership at the seam:** Which behaviors require end-to-end Python CLI tests across the Python↔Rust seam, and which can be proven only with Rust structural tests? The atoms split testing by layer, but they do not define the mandatory integration-test matrix for round-trips, store maintenance, query behavior, and command compatibility. (`testing.md`, `rust-testing.md`, `python-testing.md`)
...
```acceptance-report
{
  ...
  "commandsRun": [
    ...
    {
      "command": "functions.read on CLI, command-group, component, testing, and Rust/Python split atoms under desk/atoms",
      "result": "passed",
      "summary": "Reviewed 62 relevant atom files without reading outside desk/atoms."
    },
    {
      "command": "functions.grep for migration and Rust/Python references under desk/atoms",
      "result": "passed",
  ...
  "validationOutput": [
    ...
    "Findings are implementation-facing questions only, focused on boundaries, migration, testing scope, and the Rust/Python split.",
